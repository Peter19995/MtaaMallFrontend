...
describe('business lifecycle UI permissions', () => {
  const root = { roles: ['root_system_admin'], permissions: ['platform.businesses.read', 'platform.businesses.review', 'platform.businesses.activate', 'platform.businesses.suspend', 'platform.businesses.close'] }
  const owner = { roles: ['business_owner'], permissions: ['business.settings.read', 'business.settings.update'] }
  ...
  it('requires platform role and review permission for activation', () => {
    expect(allowedStatusChanges('pending_verification', { roles: ['business_manager'], permissions: ['platform.businesses.activate'] })).toEqual([])
    expect(allowedStatusChanges('pending_verification', { ...root, roles: ['business_owner'] })).toEqual([])
