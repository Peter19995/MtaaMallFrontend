...
import { listProductsRequest, listCategoriesRequest, type ProductResponse } from '@api/modules/products.api'
import HomePage from './HomePage'
vi.mock('@api/modules/products.api', () => ({ listProductsRequest: vi.fn(), listCategoriesRequest: vi.fn() }))
const product = { id: 1, name: 'Everyday tea', category_name: 'Groceries', price: 0, stock_quantity: 0, is_active: true } as ProductResponse
...
beforeEach(() => { vi.mocked(listProductsRequest).mockResolvedValue([product]); vi.mocked(listCategoriesRequest).mockResolvedValue([{ id: 1, name: 'Groceries' }]) })
...
it('allows retry after catalog failure', async () => {
  vi.mocked(listProductsRequest).mockRejectedValueOnce(new Error('Offline')).mockResolvedValueOnce([product])
  show()
