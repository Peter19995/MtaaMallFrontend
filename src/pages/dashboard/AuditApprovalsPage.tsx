...
const labels = { discount: 'Discounted POS sale', refund: 'POS refund', stock_writeoff: 'Stock write-off', pos_void: 'POS void', role_change: 'Role change', financial_posting: 'Financial posting' }
...
export default function AuditApprovalsPage({ approvals = false }: { approvals?: boolean }) {
  ...
  const auditFilters = { action, actor_user_id: actorId, resource_type: resourceType, branch_id: branchId }
  const approvalFilters = { status, kind }
  const audit = useQuery({ queryKey: ['audit', user?.context, offset, auditFilters], queryFn: () => listAudit(platform, offset, auditFilters), enabled: !approvals, refetchInterval: 30000 })
  const requests = useQuery({ queryKey: ['approvals', user?.context, offset, approvalFilters], queryFn: () => listApprovals(offset, approvalFilters), enabled: approvals, refetchInterval: 15000 })
  const query = approvals ? requests : audit
  return <div className="space-y-6">
    <header className="rounded-2xl bg-slate-900 p-7 text-white"><p className="text-xs uppercase tracking-widest text-slate-300">Accountability & control</p><h1 className="mt-3 text-3xl font-bold">{approvals ? 'Sensitive action approvals' : 'Audit history'}</h1><p className="mt-3 max-w-3xl text-sm text-slate-300">{approvals ? 'Review the exact requested values for operational, workforce and financial actions. A different authorized person must decide each request.' : 'Append-only events from your authorized scope. Expand an event to compare before and after values. Records cannot be edited or deleted.'}</p></header>
    ...
    <div className="flex flex-wrap items-center gap-3">{!approvals ? <><input className="rounded-lg border p-3" placeholder="Exact action" aria-label="Filter by action" value={action} onChange={e => { setAction(e.target.value); setOffset(0) }} /><input className="rounded-lg border p-3" placeholder="Resource type" aria-label="Filter by resource type" value={resourceType} onChange={e => { setResourceType(e.target.value); setOffset(0) }} /><input className="w-40 rounded-lg border p-3" placeholder="Actor user ID" aria-label="Filter by actor" inputMode="numeric" value={actorId} onChange={e => { setActorId(e.target.value); setOffset(0) }} /><input className="w-36 rounded-lg border p-3" placeholder="Branch ID" aria-label="Filter by branch" inputMode="numeric" value={branchId} onChange={e => { setBranchId(e.target.value); setOffset(0) }} /></> : <><select className="rounded-lg border bg-white p-3" aria-label="Filter approval status" value={status} onChange={e => { setStatus(e.target.value); setOffset(0) }}><option value="">All statuses</option><option value="pending">Pending</option><option value="executed">Executed</option><option value="rejected">Rejected</option></select><select className="rounded-lg border bg-white p-3" aria-label="Filter approval type" value={kind} onChange={e => { setKind(e.target.value); setOffset(0) }}><option value="">All action types</option>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></>}<button className="rounded-lg border bg-white px-4 py-3" onClick={() => query.refetch()}>Refresh</button></div>
