...
const CatalogSearchPage = ({ initialView = 'search' }: CatalogSearchPageProps) => {
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary-dark">Shared product data</p><h2 className="mt-1 text-2xl font-bold text-text sm:text-3xl">{initialView === 'proposal' ? 'Create product proposal' : 'Find existing product'}</h2><p className="mt-2 max-w-3xl text-sm text-text-secondary">Search approved products before creating duplicates. Your SKU, pricing, suppliers, stock and publication settings remain private to your business.</p></div><div className="flex flex-wrap gap-2"><Link to={`${workspaceBase}/products/add-from-catalog`} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${initialView === 'search' ? 'bg-primary text-white' : 'border border-border bg-white text-text-secondary hover:border-primary'}`}>Find existing product</Link>{canPropose && <button type="button" onClick={openProposal} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${initialView === 'proposal' ? 'bg-primary text-white' : 'border border-border bg-white text-text-secondary hover:border-primary'}`}>Create new product proposal</button>}</div></header>

    <section className="grid gap-3 md:grid-cols-2">
      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4"><p className="text-xs font-bold uppercase tracking-[.14em] text-primary-dark">Catalogue information</p><h3 className="mt-1 font-bold text-text">Shared across MtaaMall</h3><p className="mt-1 text-sm text-text-secondary">Product identity, brand, category, package details and approved identifiers are managed by the platform and are read-only here.</p></div>
      ...
      <motion.section role="dialog" aria-modal="true" aria-labelledby="catalog-link-title" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: .96 }} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-6">
          ...
          {adoptionMode === 'new' && canAdopt ? <>
            <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-primary/20 bg-primary/5 p-3"><strong className="text-sm text-text">Catalogue information</strong><p className="mt-1 text-xs text-text-secondary">Shared across MtaaMall and read-only.</p></div><div className="rounded-xl border border-secondary/20 bg-secondary/5 p-3"><strong className="text-sm text-text">Your listing</strong><p className="mt-1 text-xs text-text-secondary">SKU, price, tax and overrides belong only to this business.</p></div></div>
