...
export default function BusinessWorkforcePage() {
  ...
  const audit = useQuery({ queryKey: ['business-workforce', 'audit', user?.context], queryFn: listBusinessMembershipAudit, refetchInterval: 30000 })
  ...
  const availableRoles = user?.roles?.includes('branch_manager')
    ? roles.filter(value => value !== 'business_owner' && value !== 'business_admin')
    : roles
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-12">
    ...
    {(members.isError || invitations.isError || audit.isError || branches.isError) && <ErrorNotice error={members.error || invitations.error || audit.error || branches.error} />}
    ...
    <section className={`${card} overflow-hidden`}><div className="border-b border-slate-100 p-5 sm:p-6"><h2 className="text-lg font-semibold">Membership audit history</h2><p className="mt-1 text-sm text-slate-500">An append-only record of employee access changes visible within your branch scope.</p></div>
      {audit.isPending ? <p className="p-6" role="status">Loading audit history…</p> : <div className="divide-y divide-slate-100">{audit.data?.map(event => <article className="grid gap-2 p-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:p-6" key={event.id}><div><p className="font-semibold text-slate-900">{event.actor_name} · {nice(event.action.replaceAll('.', '_'))}</p><p className="mt-1 text-sm text-slate-600">Employee: {event.subject_name}</p><p className="mt-2 text-sm text-slate-700">{event.reason}</p></div><time className="text-xs text-slate-400" dateTime={event.created_at}>{new Date(event.created_at).toLocaleString()}</time></article>)}</div>}
      {audit.data?.length === 0 && <p className="p-8 text-center text-sm text-slate-500">No membership changes have been recorded yet.</p>}
    </section>
