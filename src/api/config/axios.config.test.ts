...
describe("active API context", () => {
  it("clears old workspace data before switching businesses", () => {
    ...
    queryClient.setQueryData(["products", "list"], ["Business A listing"]);
    ...
    expect(
      queryClient.getQueryData(["catalog", "search", "cola"]),
    ).toBeUndefined();
  ...
  it("drops tenant caches and returns revoked memberships to workspace discovery", () => {
    ...
    expect(
      queryClient.getQueryData(["business:a", "inventory"]),
    ).toBeUndefined();
    expect(replace).toHaveBeenCalledWith(
      "/account/workspaces?workspaceRefresh=12345",
    );
  });
});
